...
import { WatchedMovie, UserProfile } from "@/types"
...
export default function SettingsPage({ params }: {
  ...
  const { getById } = useAuth()
  const [user, setUser] = useState<UserProfile | null>(null)
  const { id, username, language, created_at, profile_picture } = user || {}
          ...
          <CardHeader className="pb-0">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              <Avatar className="h-24 w-24 border-4 border-zinc-800 shadow-md">
                <AvatarImage src={profile_picture} alt="Profile Picture" className="object-cover" />
